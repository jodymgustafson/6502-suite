...
## Command Line
You can run the assembler and disassembler from the command line.

### Assembler command line
Takes the name of a file containing assembly code and optionally the base address (see assembler section below).

`node assemble test.asm '$0100'`

### Disassembler command line
Takes the name of a file containing bytes of machine code and optionally the base address (see disassembler section below).
The bytes should be hex codes separated by whitespace (e.g. `A0 00 A2 00 E8`).

`node disassemble test.bin '$0100'`

## Assembler Module
The assembler takes a string of assembly code and turns it into an array of machine code bytes.

`assemble(code: string, baseAddr?: number): number[]`
...
In addition to the standard 6502 op codes this assembler supports the following pseudo-commands:
...
## Emulator Module
The emulator is an object takes an array of machine code bytes (generated by the assembler) and executes them.
...
The emulator provides the following callbacks:
- onStop - Called when the emulator has stopped
- onStep - Called each time an instruction has finished
- onBatch - Called when the processor completes a batch of instruction
...
The callback will receive one parameter which is the execution result (see step() method above).
...
Otherwise the emulator will nor be able to perform at the speed requested.
